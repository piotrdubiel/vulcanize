...
var setAttribute = utils.setAttribute;
...
function removeCommentsAndWhitespace($) {
  function walk(node) {
    ...
    if (!node) {
      ...
      // only run uglify on inline javascript/coffeescript scripts
      if (!node.attribs.src) {
          if (!node.attribs.type || node.attribs.type == "text/javascript") {
            content = getTextContent(node);
            setTextContent(node, compressJS(content, true));
          }
          else if (node.attribs.type == "text/coffeescript") {
            content = getTextContent(node);
            setTextContent(node, compressJS(content, true));
          }
      }
